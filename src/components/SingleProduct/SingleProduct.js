...
const SingleProduct = () => {
  ...
  const modalOverlayHandler = (e) => {
    if(e.target.classList.contains('overlay-bg')|| e.key === 'Backspace') {
      document.addEventListener('keydown', modalOverlayHandler);
      dispatch(setIsModalVisible(false));
    }
  }
    ...
    <div className='overlay-bg' onClick = {modalOverlayHandler}>
      <div className = "product-details-modal">
        ...
        <div className = "details-content grid">
          ...
          {/* detials right */}
          {
           !!product && (<div className='details-left'>
            <div className = "details-info">
              <h3 className = "title fs-22 fw-5">{product.title}</h3>
              <p className='description text-regal-blue'>{product.description}</p>
